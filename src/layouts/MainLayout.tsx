...
<div className="flex gap-4 justify-center">
                  {[Globe, MessageCircle, Share2].map((Icon, idx) => {
                    const socialLabels = ['website', 'whatsapp', 'share'];
                    return (
                      <div
                        key={idx}
                        onClick={() => trackSocialClick(socialLabels[idx])}
                        className="w-12 h-12 rounded-full bg-[#111827] border border-[#FF8A00]/20 flex items-center justify-center text-[#FFD54A] shadow-[0_0_10px_rgba(255,138,0,0.1)] cursor-pointer"
                      >
                        <Icon size={20} />
                      </div>
                    );
            ...
            <div className="lg:col-span-5">
              <Link to="/" className="flex items-center gap-3 mb-6 group">
                ...
                <div className="flex flex-col">
                  ...
                  <span className="text-[10px] uppercase tracking-[0.3em] text-[#FFD54A] font-bold mt-1">Hunar Bazaar 2026</span>
                </div>
              </Link>
...
<div className="flex gap-4">
                {[Globe, MessageCircle, Share2, Mail].map((Icon, idx) => {
                  const socialLabels = ['website', 'whatsapp', 'share', 'email'];
                  return (
                    <a
                      key={idx}
                      href="#"
                      onClick={(e) => { e.preventDefault(); trackSocialClick(socialLabels[idx]); }}
                      className="w-11 h-11 rounded-xl bg-white/5 border border-white/10 flex items-center justify-center text-gray-500 hover:bg-[#FF8A00] hover:text-[#07111F] hover:border-[#FF8A00] transition-all duration-300 hover:-translate-y-1"
                    >
                      <Icon size={18} />
                    </a>
                  );
...
<Link to="/register" onClick={() => trackRegisterClick('footer')} className="btn-premium w-full justify-between group py-3.5 text-sm">
          ...
          <div className="flex flex-col md:flex-row justify-between items-center gap-6">
            <p className="text-gray-600 font-bold uppercase text-[10px] tracking-widest">
              &copy; 2026 हुनर बाजार · Hunar Bazaar. All rights reserved.
            </p>
